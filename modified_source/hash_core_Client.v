...
assign hash_fifo_ready = fifo_rst_ready_ctr == 3'h5;
assign ififo_req = hash_fifo_ready & ~(ififo_empty|last&~last_r1|pad_flag|keccak_busy);
assign pad_last = pad_ctr == {mode[1:0],3'h7};

always @(posedge clk) begin
	if(rst)
		fifo_rst_ready_ctr <= 3'h0;
	else if(!hash_fifo_ready)
		fifo_rst_ready_ctr <= fifo_rst_ready_ctr + 1'h1;
	else
		fifo_rst_ready_ctr <= fifo_rst_ready_ctr;
end
...
fifo_generator_0 fifo0(.clk(clk),.srst(rst),.din({ififo_mode,ififo_absorb,ififo_last,ififo_din}),.wr_en(hash_fifo_ready & ififo_wen),.rd_en(ififo_req),.dout({mode,absorb,last,ififo_dout}),.full(ififo_full),.empty(ififo_empty)); 
