...
reg [255:0] d, rho, sigma, hash_c, K;
(* shreg_extract = "no" *) reg [255:0] hash_pk;
reg [255:0] m, z;
wire [1:0] m_dec;
wire m_ena;
...
assign start_launch = state == 6'h0 && hash_fifo_ready && (start_pending || start);

always @* case(state)
	6'h 0 : next_state = start_launch ? state + 1'h 1 : state;
...
endcase
always @(posedge clk) begin
	if(start_launch | CCA_enc_start) begin
		patt_r <= patt;
		eta3_r <= eta3;
		endp_r <= endp;
	end
...
.hash_fifo_ready(hash_fifo_ready),
